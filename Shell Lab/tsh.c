 ...
 * <Put your name and login ID here>
   Julia Sanford - jusa1470@colorado.edu
 ...
 */
int parseline(const char *cmdline, char **argv) //don't have to do
{
 ...
 */
int is_builtin_cmd(char **argv) //10 lines - done i think
{
    ...
    else return BLTN_UNK;     /* not a builtin command */
}

/*
 * do_exit - Execute the builtin exit command
 */
void do_exit(void) //1 line - done
{
    exit(0);
    return;
}
 ...
 */
void do_show_jobs(void) //1 line - done i think
{
    showjobs(jobs);
    return;
}
 ...
 */
void do_ignore_singleton(void) //1 line
{
    printf("Ignoring singleton '&'!");
    return;
}

void do_killall(char **argv) //8 lines
{
 ...
 * EXISTING (pid != 0) job
 */
void sigalrm_handler(int sig) //15
{
